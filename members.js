...
async function openMemberEditor(id){
  try{
    const recs=await api('get_member',{params:{id:String(id)}}); const m=Array.isArray(recs)?recs[0]:null; 
    if(!m) throw new Error('Δεν βρέθηκε το μέλος.');
    editingMember=m;
    editFirst.value=m.first_name||''; editLast.value=m.last_name||''; editEpoch.value=(m.counter_epoch||todayISO()).slice(0,10);
